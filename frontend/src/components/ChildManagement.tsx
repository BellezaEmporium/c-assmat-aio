...
export const ChildManagement = () => {
  ...
  const fetchChildren = async () => {
    try {
      const response = await axios.get(`${API_URL}/me/children`, AXIOS_CONFIG);
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get(`${API_URL}/me/children`, AXIOS_CONFIG),
      axios.get(`${API_URL}/me/employers`, AXIOS_CONFIG),
    ]).then(([childrenResponse, employersResponse]) => {
      if (cancelled) return;
  ...
  const handleSave = async () => {
    try {
      if (currentChild.id) {
        await axios.put(`${API_URL}/me/children/${currentChild.id}`, currentChild, AXIOS_CONFIG);
      } else {
        await axios.post(`${API_URL}/me/children`, currentChild, AXIOS_CONFIG);
      }
      fetchChildren();
      handleClose();
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`${API_URL}/me/children/${id}`, AXIOS_CONFIG);
      fetchChildren();
    } catch (error) {
